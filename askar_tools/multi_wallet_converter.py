...
class MultiWalletConverter:
    ...
    async def _add_list_of_entries_to_wallet(self, store, entries):
        async with store.transaction() as txn:
            try:
                for entry in entries:
                    if await txn.fetch(entry.category, entry.name):
                        print(f"Replacing {entry}")
                        await txn.replace(
                            entry.category, entry.name, entry.value, entry.tags
                        )
                    else:
                ...
                print(f"Error adding entries to wallet: {e}")
                await txn.rollback()
                raise ConversionError("Error adding entries to wallet")
    ...
    async def convert_single_wallet_to_multi_wallet(self):
        ...
        for wallet_record in self.get_wallet_records(admin_store_entries):
            try:
                ...
                # Get the store and entries from the sub wallet
                ...
                sub_wallet_tenant_store_scan = sub_wallet_tenant_store.scan()
                sub_wallet_tenant_store_entries = (
                    await sub_wallet_tenant_store_scan.fetch_all()
                )

                # Copy the base store to the new stand-alone store
                stand_alone_tenant_store = await sub_wallet_tenant_store.copy_to(
                ...
                await self._add_list_of_entries_to_wallet(
                    stand_alone_tenant_store, sub_wallet_tenant_store_entries
                )
                await stand_alone_tenant_store.close()
            except Exception as e:
                print(
